<x-layouts.admin :header="'Edit Buku'">
    <div class="max-w-5xl">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
            ...
            <h2 class="text-xl font-headline font-bold text-[#1b1c19] truncate max-w-sm" title="{{ $book->judul }}">{{ $book->judul }}</h2>
        </div>
        ...
        <form method="POST" action="{{ route('admin.books.update', $book) }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    <div class="bg-white rounded-card p-6" style="box-shadow: 0 2px 12px rgba(26,77,46,0.04);">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="judul" value="{{ old('judul', $book->judul) }}" required class="input-field text-lg font-semibold">
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input type="text" name="isbn" value="{{ old('isbn', $book->isbn) }}" class="input-field font-mono text-sm">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-card p-6" style="box-shadow: 0 2px 12px rgba(26,77,46,0.04);">
                        ...
                        <textarea name="deskripsi" rows="5" class="input-field leading-relaxed">{{ old('deskripsi', $book->deskripsi) }}</textarea>
                    </div>
                </div>
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="stok" value="{{ old('stok', $book->stok) }}" required class="input-field text-center font-bold text-lg" min="0">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold uppercase tracking-wider text-[#1a4d2e] mb-2">Lokasi Rak</label>
                                    <input type="text" name="lokasi_rak" value="{{ old('lokasi_rak', $book->lokasi_rak) }}" class="input-field text-center font-bold">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-card p-6" style="box-shadow: 0 2px 12px rgba(26,77,46,0.04);">
                        ...
                        <div class="flex flex-col items-center">
                            ...
                            <div class="relative group mb-4">
                                <img src="{{ Storage::url($book->cover) }}" alt="Cover" class="w-32 h-44 object-cover rounded shadow-md transition-transform group-hover:scale-105">
                            </div>
                            @else
            ...
            <div class="flex justify-end gap-4 mt-8 bg-white p-4 rounded-card border border-[#eae8e3] items-center" style="box-shadow: 0 -4px 12px rgba(26,77,46,0.02);">
                ...
                <button type="submit" class="btn-primary px-8 py-2.5 text-sm">Simpan Perubahan Buku</button>
            </div>
        </form>
    </div>
</x-layouts.admin>